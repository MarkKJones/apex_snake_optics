...
#include <TPolyLine.h>
#include <TLegend.h>
void plot_apex_Rhrs(TString fstart) {
  gROOT->Reset();
  gStyle->SetOptStat(0);
 ...
 Double_t yp_cent=5./180.*3.14159;
 cout << " yp_cent = " << yp_cent << endl;
...
TFile *hsnake = new TFile("rootfiles/"+fstart+"_standard_hist.root","recreate");
 Bool_t fit_matrix=kTRUE;
   string newcoeffsfilename="matrix-files/hms_newfit6_Rhrs_save.dat";
 TRandom3 err; 
//
...
Double_t evnum,epnum,xabs,yabs,zabs,cxabs,cyabs,czabs,mom,pathl,live,xrel,yrel,zrel,cxrel,cyrel,czrel;
 ...
 TH1F *hDeltadiff = new TH1F("hDeltadiff","Delta Diff % ",200,-.3,.3);
 TH1F *hDeltanew = new TH1F("hDeltanew","Delta New Recon ",40,-.12,.12);
 TH1F *hDeltanewdiff = new TH1F("hDeltanewdiff","Delta New Diff % ",200,-.3,.3);
 ...
 TH1F *hztar = new TH1F("hztar","ztar (cm)",100,-35.,35.);
 TH1F *hztarrecon = new TH1F("hztarrecon","ztar recon(cm)",100,-35.,35.);
 TH1F *hytarnew = new TH1F("hytarnew","ytar new(cm)",70,-7.,7.);
 TH1F *hytardiff = new TH1F("hytardiff","ytar diff(cm)",200,-5.,5.);
 TH1F *hytarnewdiff = new TH1F("hytarnewdiff","ytar new diff(cm)",200,-5.,5.);
 ...
 //
 TH2F *hxyfp = new TH2F("hxyfp","xfp vs yfp (cm)",40,-5.,5.,100,-100.,100.);
...
TH2F *hxpyfp = new TH2F("hxpyfp","xpfp vs yfp ",100,-40.,40.,100,-.1,.1);
 ...
 TH2F *hyfpvsydiff = new TH2F("hyfpvsydiff","y_fp vs ydiff; y_fp (cm); ytar diff (cm) ",50,-20.,20.,40,-2,2.);
 ...
 //
 //
const int NumEndPl=28;
const int NumFocalPlane=26;
...
,+80.,+80.
 ...
 TString EndPlname[NumEndPl]={
"Target"," 78.0cm from target"," Sieve"," Apex 	-61cm","Apex +70cm"
,"before Q1 ent ","Q1 ent 47.0 "," Q1 exit 94cm","Q1Q2 61.04cm","Q1Q2 67.5cm"
,"Q1Q2 117.0cm","Q2 91.5","Q2 center " ,"Q2 56.17","Q2 85.5"
,"Q2D -304 cm","Q2D -150.","DIP -2681.2","DIP 1512cm","Dip center"
,"DQ3 115.4 ","Q3 -57.5","Q3 91.5","Q3 exit ","Q3 57.5 ",
...
//
 ...
 Double_t xptar,yptar,xpfp,ypfp,xtar,ztar,xfp,yfp;
 int ntracks=0;
 ...
 int nfit=0;
      nentries = (int)tsnake->GetEntries();
           for (int ie = 0; ie < nentries; ie++)
            {
	      if (ie%10000==0)    	      cout << ie << " ntracks = " << ntracks << endl;
	         tsnake->GetEntry(ie);
   	  delta=(mom-cmom)/cmom;
          EndPl = epnum;
          if ( EndPl > NumEndPl) {
	...
	if ( epnum == 0  ) {
	  //	  cout << " evnum = " << evnum << " " << ie<< " " << nentries<< endl;
          ntracks++;
          yptar=yp_cent-abs(czabs/cyabs);
	  xptar=cxabs/cyabs;
          ztar=(yabs)/10.;
	  hztar->Fill(ztar);
          ytar=-ztar/((cos(thcent)/tan(thcent-yptar)+sin(thcent)));
	  xtar=-xptar*ztar*cos(thcent)/100.;
          hMom->Fill(mom);
          hDelta->Fill(delta);
          hytar->Fill(ytar);
	  hxptar->Fill(xptar);
	...
	if ( live == 1 && epnum==NumFocalPlane   ) {
	  ...
	  */
	  ...
	  hxpxfp->Fill(xpfp,xfp);
	  hxpyfp->Fill(yfp,xpfp);
	  hypyfp->Fill(ypfp,yfp);
...
//
    ...
    c2b->cd(2);
    hztar->Draw();
    hztarrecon->Draw("same");
    hztarrecon->SetLineColor(2);
