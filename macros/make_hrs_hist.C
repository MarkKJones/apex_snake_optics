...
TFile *hsnake = new TFile("rootfiles/"+fstart+"_hist.root","recreate");
 ...
 Bool_t reached_fp=kTRUE;
   string newcoeffsfilename="apex_ver2.dat";
 TRandom3 err; 
//
...
//
 ...
 int nfit;
	...
	if ( live == 1 && epnum==NumFocalPlane   ) {
	  ...
	  if ( TMath::Abs(ytar+2.4) < .1) hxpyp->Fill(yptar,xptar);
	  hypdel->Fill(delta,yptar);
	  if ( TMath::Abs(ytar) < .1) hypytar->Fill(ytar,yptar);
	  if ( TMath::Abs(ytar-1.2) < .1) hypytar->Fill(ytar,yptar);
	  if ( TMath::Abs(ytar-2.4) < .1) hypytar->Fill(ytar,yptar);
	  if ( TMath::Abs(ytar+1.2) < .1) hypytar->Fill(ytar,yptar);
	  if ( TMath::Abs(ytar+2.4) < .1) hypytar->Fill(ytar,yptar);
	  ...
	  if ( TMath::Abs(ytar) < .1) hDeltadiffvsxpfp->Fill(100*(deltatemp-delta),xpfp);         
	  ...
	  if ( TMath::Abs(ytar+2.4) < .1) hxpyprecon->Fill(yptartemp,xptartemp);
	  if ( TMath::Abs(ytar) < .1) hypytarrecon->Fill(ytartemp*100,yptartemp);
	  if ( TMath::Abs(ytar-1.2) < .1) hypytarrecon->Fill(ytartemp*100,yptartemp);
	  if ( TMath::Abs(ytar-2.4) < .1) hypytarrecon->Fill(ytartemp*100,yptartemp);
	  if ( TMath::Abs(ytar+1.2) < .1) hypytarrecon->Fill(ytartemp*100,yptartemp);
	  if ( TMath::Abs(ytar+2.4) < .1) hypytarrecon->Fill(ytartemp*100,yptartemp);
	  
          if (TMath::Abs(ytar) < .05) hypreconvxprecon->Fill(xptartemp,yptartemp);       
	}
	//        
	    }
// end loop over tree
