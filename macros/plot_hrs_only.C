...
#include <TPolyLine.h>
#include <TLegend.h>
void plot_hrs_only(TString fstart) {
  gROOT->Reset();
  gStyle->SetOptStat(0);
 ...
 Double_t cmom=0.837595,delta,ytar,ztemp,thcent=20./180.*3.14159;
...
TFile *hsnake = new TFile("rootfiles/"+fstart+"_standard_hist.root","recreate");
 Bool_t fit_matrix=kFALSE;
   string newcoeffsfilename="matrix-files/hms_newfit1_save.dat";
 TRandom3 err; 
//
  string oldcoeffsfilename="matrix-files/hms_newfit_save.dat";
...
Double_t evnum,epnum,xabs,yabs,zabs,cxabs,cyabs,czabs,mom,pathl,live,xrel,yrel,zrel,cxrel,cyrel,czrel;
 ...
 TH2F *hxyfp = new TH2F("hxyfp","xfp vs yfp (cm)",40,-20.,20.,100,-100.,100.);
 ...
 TH2F *hypyfp = new TH2F("hypyfp","yfp vs ypfp ",100,-.06,.06,100,-20.,20.);
 ...
 int nfit=0;
      nentries = (int)tsnake->GetEntries();
           for (int ie = 0; ie < nentries; ie++)
            {
	      if (ie%50000==0)    	      cout << ie << " nfit = " << nfit << endl;
	         tsnake->GetEntry(ie);
   	  delta=(mom-cmom)/cmom;
          EndPl = epnum;
          if ( EndPl > NumEndPl) {
